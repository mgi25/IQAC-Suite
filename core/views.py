...
def api_cdl_head_dashboard(request):
    ...
    try:
        ...
        cdl_members = [
            user.get_full_name() or user.username for user in cdl_group.user_set.all()
        ]
        if not cdl_members:
            cdl_members = []
    ...
    data = {
        ...
        "assigned_to_name": (
            ...
            if asg
                ...
                p.report_assigned_to.get_full_name() if p.report_assigned_to else None
            )
        ),
...
def api_cdl_member_work(request):
    ...
    """
    me = request.user
    qs = EMTEventProposal.objects.filter(report_assigned_to=me).select_related(
        "organization"
    )[:100]
    items = []
    for p in qs:
        items.append(
            {
                ...
                "type": (
                    "poster"
                    if getattr(
                        getattr(p, "cdl_support", None), "poster_required", False
                    )
                    else (
                        "certificate"
                        if getattr(
                            getattr(p, "cdl_support", None),
                            "certificates_required",
                            False,
                        )
                        else "coverage"
                ...
                or (p.event_datetime.date().isoformat() if p.event_datetime else None),
                "status": "pending",
                "rev": 0,
                "created_at": p.created_at.isoformat(),
                "assigned_at": (
...
def api_cdl_member_data(request):
    ...
    asg_ids = list(
        _CDLAssignment.objects.filter(assignee=user).values_list(
            "proposal_id", flat=True
        )
    )
    legacy_qs = EMTEventProposal.objects.filter(report_assigned_to=user).values_list(
        "id", flat=True
    )
    proposal_ids = list(set(asg_ids) | set(legacy_qs))
    work_qs = EMTEventProposal.objects.filter(id__in=proposal_ids).select_related(
        "organization"
    )[:200]
    work = []
    due_today = 0
        ...
        work.append(
            {
                ...
                "assigned_at": (
                    ...
                    if _asg
                        ...
                        p.report_assigned_at.isoformat()
                        if p.report_assigned_at
                        else None
                    )
                ),
