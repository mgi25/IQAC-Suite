...
from .forms import (CDLMessageForm, CDLRequestForm, RegistrationForm,
                    RoleAssignmentForm)
from .models import (ActivityLog, ApprovalFlowConfig, ApprovalFlowTemplate,
                     CDLCommunicationMessage, CDLCommunicationThread,
                     CDLRequest, CertificateBatch, CertificateEntry, Class,
                     FacultyMeeting, Organization, OrganizationRole,
                     OrganizationType, Profile, Program, Report,
                     RoleAssignment)

logger = logging.getLogger(__name__)

EMTEventProposal = EventProposal
EMTEventReport = EventReport
    ...
    return JsonResponse(
        {
            ...
            "description": (
                getattr(getattr(p, "objectives", None), "content", "") or ""
            ).strip(),
        }
    )
        ...
        items.append(
            {
                ...
                "assignee_name": (
                    (t.assignee.get_full_name() or t.assignee.username)
                    if t.assignee_id
                    else None
                ),
            }
...
def api_cdl_events_my_support(request):
    me = request.user
    qs = (
        EventProposal.objects.filter(submitted_by=me, cdl_support__needs_support=True)
...
def custom_logout(request):
    ...
    return redirect(
        f"{google_logout_url}?continue=https://appengine.google.com/_ah/logout?"
        "continue=http://127.0.0.1:8000/accounts/login/"
    )
...
def my_profile(request):
    ...
    roles = RoleAssignment.objects.filter(user=user).select_related(
        "role", "organization"
    )
    ...
    if is_student:
        ...
        context.update(
            {
                ...
                "profile_completion_percentage": calculate_student_profile_completion(
                    user, context
                ),
            }
        )
        ...
        template = (
            "core/student_profile.html" if from_dashboard else "core/my_profile.html"
        )
        ...
        context.update(
            {
                ...
                "user_organizations": Organization.objects.filter(
                    id__in=roles.filter(organization__isnull=False).values_list(
                        "organization_id", flat=True
                    )
                ),
                "years_active": calculate_years_active(user),
                "profile_completion_percentage": calculate_faculty_profile_completion(
                    user, context
                ),
            }
        )
        ...
        template = (
            "core/faculty_profile.html" if from_dashboard else "core/my_profile.html"
        )
...
def api_cdl_head_dashboard(request):
    ...
    assets_pending = (
        ...
        .filter(
            models.Q(cdl_support__poster_required=True)
            | models.Q(cdl_support__certificates_required=True)
        )
        .filter(
            status__in=[
                EventProposal.Status.SUBMITTED,
        ...
        events.append(
            {
                ...
                "type": (
                    "cdl_support"
                    if getattr(getattr(p, "cdl_support", None), "needs_support", False)
                    else "proposal"
                ),
            }
        )
        ...
        event_details.append(
            {
                ...
                "certificates_required": bool(
                    getattr(cs, "certificates_required", False)
                ),
            }
        )
    ...
    # Prefer RoleAssignment-based discovery (more robust across setups)
    try:
        from .models import RoleAssignment

        ras = (
            RoleAssignment.objects.select_related(
                "role", "organization", "organization__org_type"
            )
            .filter(organization__org_type__name__iexact="CDL")
        )
        # Accept common CDL role names (case-insensitive)
        allowed_roles = {"cdl member", "cdl employee", "cdl team"}
        member_users = [
            ra.user
            for ra in ras
            if ra.user and ra.role and ra.role.name and ra.role.name.lower() in allowed_roles
        ]
    except Exception:
        member_users = []

    # Fallback: Django auth group named "CDL_MEMBER"
    if not member_users:
        try:
    ...
    if member_users:
        try:
            ...
            counts_map = {u.pk: 0 for u in member_users}
            agg = (
                CDLAssignment.objects.filter(assignee__in=member_users)
                .values("assignee")
                .annotate(c=Count("id"))
            )
            for row in agg:
                counts_map[row["assignee"]] = row["c"]
            assignments_counts = [counts_map[u.pk] for u in member_users]
        except Exception:
            # If CDLAssignment isn't available (migrations pending, etc.), default to zeros
            assignments_counts = [0] * len(member_users)
    else:
        assignments_counts = []

    workload = {
        "members": labels,
        "assignments": assignments_counts,
    }
...
def registration_form(request):
    ...
    return render(
        request, "core/Registration_form.html", {"form": form, "is_student": is_student}
    )
...
def dashboard(request):
    ...
    is_cdl_admin = any(
        ...
        and (
            ra.organization
            and ra.organization.org_type
            and ra.organization.org_type.name.lower() == "cdl"
        )
        for ra in roles
    )
    is_cdl_employee = any(
    ...
    try:
        ...
        if is_student:
            ...
            upcoming_events_count = (
                ...
                .filter(Q(event_datetime__gte=now_dt) | Q(event_start_date__gte=today))
                .count()
            )
        else:
        ...
        this_week_events = finalized_events.filter(
            event_datetime__date__gte=week_start, event_datetime__date__lte=week_end
        ).count()

        students_participated = (
            finalized_events.aggregate(
        ...
        user_proposals = EventProposal.objects.filter(submitted_by=user).order_by(
            "-updated_at"
        )[:5]
        ...
        calendar_events = [
            {
                ...
                "submitted_by": e.submitted_by.get_full_name()
                or e.submitted_by.username,
                "participants": e.fest_fee_participants or e.conf_fee_participants or 0,
                "is_my_event": user
                in [e.submitted_by] + list(e.faculty_incharges.all()),
            }
            for e in all_events
        ]
    ...
    recent_activity = []
    proposals_min = [
        {"title": p.event_title, "status": p.get_status_display()}
...
def event_contribution_data(request):
    ...
    for role_assignment in organizations:
        ...
        user_events_query = Q(organization=org) & (
            ...
            )  # User is a participant through Student profile
        )

        user_events_count = (
            EventProposal.objects.filter(user_events_query).distinct().count()
        )
        ...
        chart_data.append(
            {
                ...
                "role": (
                    role_assignment.role.name if role_assignment.role else "Unknown"
                ),
            }
        )
    ...
    overall_percentage = (
        (total_user_events / total_events * 100) if total_events > 0 else 0
    )
...
def propose_event(request):
    ...
    if request.method == "POST":
        ...
        if org_type_name and org_name:
            ...
            try:
                ...
                messages.error(
                    request, f"Organization type '{org_type_name}' does not exist."
                )
                return redirect("propose_event")
            organization, _ = Organization.objects.get_or_create(
                name=org_name, org_type=org_type_obj
            )
    ...
    return render(
        request, "core/event_proposal.html", {"organization_types": org_types}
    )
...
def proposal_status(request, pk):
    ...
    return render(
        request, "core/proposal_status.html", {"proposal": proposal, "steps": steps}
    )
...
def admin_dashboard(request):
    ...
    pending_event_reports = EventReport.objects.filter(
        Q(iqac_feedback__isnull=True) | Q(iqac_feedback="")
    ).count()

    # We assume a report is 'reviewed' if 'iqac_feedback' is NOT empty.
    reviewed_event_reports = (
        EventReport.objects.filter(iqac_feedback__isnull=False)
        .exclude(iqac_feedback="")
        .count()
    )
    ...
    stats = {
        ...
        "departments": Organization.objects.filter(
            org_type__name__icontains="department", is_active=True
        ).count(),
        "clubs": Organization.objects.filter(
            org_type__name__icontains="club", is_active=True
        ).count(),
    ...
    recent_activities = []
    recent_proposals = EventProposal.objects.select_related("submitted_by").order_by(
        "-created_at"
    )[:5]
    for proposal in recent_proposals:
        submitter_name = (
            proposal.submitted_by.get_full_name() if proposal.submitted_by else ""
        )
        event_title = getattr(
            proposal,
            "event_title",
            getattr(proposal, "title", "Untitled Event"),
        )
        recent_activities.append(
            {
                "type": "proposal",
                "title": f"Proposal by {submitter_name}",
                "description": f"New event proposal: {event_title}",
                "user": submitter_name,
                "timestamp": proposal.created_at,
                "status": proposal.get_status_display(),
            }
        )
    recent_reports = Report.objects.select_related("submitted_by").order_by(
        "-created_at"
    )[:3]
    ...
    calendar_event_proposals = EventProposal.objects.filter(
        status__in=["approved", "finalized"], event_start_date__isnull=False
    ).select_related("organization")
    events_list = []
...
def admin_role_management(request, organization_id=None):
    ...
    if organization_id:
        ...
        base_qs = (
            OrganizationRole.all_objects if show_archived else OrganizationRole.objects
        )
        ...
        if org_type_id:
            ...
            base_qs = (
                OrganizationRole.all_objects
                if show_archived
                else OrganizationRole.objects
            )
            role_names_qs = base_qs.filter(
                organization__org_type=org_type, organization__is_active=True
            )
            if show_archived:
                role_names_qs = role_names_qs.filter(status="archived")
            unique_role_names = (
                role_names_qs.values_list("name", flat=True).distinct().order_by("name")
            )
            ...
            context = {"org_types": org_types, "step": "org_types"}
            return render(request, "core/admin_role_management.html", context)
...
def add_org_role(request, organization_id):
    ...
    if not name:
        ...
        return redirect(
            f"{reverse('admin_role_management')}?org_type_id={org.org_type.id}"
        )

    # Check if this role already exists in ANY organization of this type
    existing_role = OrganizationRole.objects.filter(
        organization__org_type=org.org_type, name__iexact=name
    ).first()
    if existing_role:
        messages.warning(
            request, f"Role '{name}' already exists for {org.org_type.name}s."
        )
        return redirect(
            f"{reverse('admin_role_management')}?org_type_id={org.org_type.id}"
        )
    ...
    all_orgs_of_type = Organization.objects.filter(
        org_type=org.org_type, is_active=True
    )

    for target_org in all_orgs_of_type:
        if not OrganizationRole.objects.filter(
            organization=target_org, name__iexact=name
        ).exists():
            OrganizationRole.objects.create(
                organization=target_org, name=name, is_active=True
            )
...
def add_role(request):
    ...
    for org in orgs:
        if not OrganizationRole.all_objects.filter(
...
def update_org_role(request, role_id):
    ...
    updated_count = OrganizationRole.objects.filter(
        organization__org_type=org_type, name__iexact=old_name
...
def delete_org_role(request, role_id):
    ...
    roles_qs = OrganizationRole.all_objects.filter(
        organization__org_type=org_type, name__iexact=role_name
    )
    archived_count = 0
    for r in roles_qs:
        if r.status != r.Status.ARCHIVED:
            ...
            archived_count += 1
    messages.success(
        request, f"Role '{role_name}' archived for {archived_count} {org_type.name}(s)."
    )
    return redirect(f"{reverse('admin_role_management')}?org_type_id={org_type.id}")
...
def admin_user_management(request):
    ...
    users_list = (
        ...
        .prefetch_related(
            "role_assignments__organization__org_type", "role_assignments__role"
        )
    ...
    if org_type_ids:
        users_list = users_list.filter(
            role_assignments__organization__org_type_id__in=org_type_ids
        )
    ...
    if role_ids:
        all_roles = (
            all_roles | OrganizationRole.objects.filter(id__in=role_ids)
        ).distinct()
...
def api_admin_search_users(request):
    ...
    for user in paginated_users:
        ...
        for ra in user.role_assignments.all():
            ...
            if ra.organization:
                organizations.append(
                    (
                        "<div>"
                        f"{ra.organization.name} "
                        f'<small class="text-muted">({ra.organization.org_type.name})</small>'
                        "</div>"
                    )
                )
        ...
        edit_button = (
            f'<a href="/core-admin/users/{user.id}/edit/" '
        ...
        if user.id != request.user.id:
            impersonate_button = (
                f'<a href="/core-admin/impersonate/{user.id}/?next={dashboard_url}" '
                'class="btn btn-sm btn-outline-secondary ms-1">'
                '<i class="fas fa-user-secret"></i> Login as</a>'
            )
            action_buttons += impersonate_button
        data.append(
            {
                "s_no": start + len(data) + 1,
                "name": (
                    f"<strong>{user.get_full_name() or user.username}</strong>"
                    f'<small class="text-muted d-block">@{user.username}</small>'
                ),
                "email": (
                    f'<a href="mailto:{user.email}">{user.email}</a>'
                    if user.email
                    else '<span class="text-muted">No email</span>'
                ),
                "roles": "<br>".join(roles)
                or '<span class="text-muted">No roles</span>',
                "organization": "<br>".join(organizations)
                or '<span class="text-muted">No assignments</span>',
...
def admin_user_edit(request, user_id):
    ...
    org_ids = list(
        user.role_assignments.exclude(organization__isnull=True).values_list(
            "organization_id", flat=True
        )
    )
    org_qs = Organization.objects.filter(
        Q(is_active=True) | Q(id__in=org_ids)
    ).select_related("org_type")
    role_ids = list(user.role_assignments.values_list("role_id", flat=True))
    role_qs = OrganizationRole.objects.filter(
        Q(is_active=True) | Q(id__in=role_ids)
    ).select_related("organization")
    ...
    roles_json = {
        str(r.id): {"name": r.name, "organization": str(r.organization_id)}
        for r in role_qs
    }
...
def admin_reports(request):
    reports = (
        Report.objects.select_related("submitted_by").all().order_by("-created_at")
    )
...
def restore_org_role(request, role_id):
    ...
    roles_qs = OrganizationRole.all_objects.filter(
        organization__org_type=org_type, name__iexact=role_name, status="archived"
    )
    restored = 0
    for r in roles_qs:
        r.restore()
        restored += 1
    messages.success(
        request, f"Role '{role_name}' restored for {restored} {org_type.name}(s)."
    )
    return redirect(
        f"{reverse('admin_role_management')}?org_type_id={org_type.id}&archived=1"
    )
...
STATUSES = [
    ...
    "rejected",
    "finalized",
]  # ensure this matches your status choices


def iqac_suite_dashboard(request):
    user_proposals = EventProposal.objects.filter(submitted_by=request.user).order_by(
        "-created_at"
    )

    for proposal in user_proposals:
        proposal.statuses = STATUSES
        proposal.status_index = (
            STATUSES.index(proposal.status) if proposal.status in STATUSES else -1
        )
        proposal.progress_percent = (
            ((proposal.status_index + 1) / len(STATUSES)) * 100
            if proposal.status_index != -1
            else 0
        )
...
def admin_master_data(request):
    ...
    for org_type in org_types:
        ...
        orgs_by_type_json[org_type.name.lower()] = [
            {"id": o.id, "name": o.name} for o in active_orgs
        ]

    return render(
        request,
        "core/admin_master_data.html",
        {
            "org_types": org_types,
            ...
            if model_name == "organization" and hasattr(obj, "parent"):
                if parent_id and parent_id.strip():
                    try:
                        ...
                        if (
                            obj.org_type.parent_type
                            and parent_obj.org_type != obj.org_type.parent_type
                        ):
                            return JsonResponse(
                                {
                                    "success": False,
                                    "error": (
                                        "Invalid parent type. "
                                        f"Expected {obj.org_type.parent_type.name}, "
                                        f"got {parent_obj.org_type.name}"
                                    ),
                                }
                            )
        ...
        try:
            ...
            if model_name == "organization":
                ...
                if not org_type_name:
                    return JsonResponse(
                        {"success": False, "error": "Organization type required"}
                    )

                try:
                    org_type_obj = OrganizationType.objects.get(
                        name__iexact=org_type_name
                    )
                except OrganizationType.DoesNotExist:
                    return JsonResponse(
                        {
                            "success": False,
                            "error": f"Organization type '{org_type_name}' does not exist",
                        }
                    )

                if Organization.objects.filter(
                    name__iexact=name, org_type=org_type_obj
                ).exists():
                    return JsonResponse(
                        {
                            "success": False,
                            "error": f"{org_type_obj.name} with name '{name}' already exists",
                        }
                    )
                ...
                obj = Organization.objects.create(
                    name=name, org_type=org_type_obj, parent=parent_obj, is_active=True
                )
                ...
                if OrganizationType.objects.filter(name__iexact=name).exists():
                    return JsonResponse(
                        {
                            "success": False,
                            "error": f"Organization type '{name}' already exists",
                        }
                    )
                ...
                if parent_id:
                    try:
                        ...
                        return JsonResponse(
                            {
                                "success": False,
                                "error": "Parent organization type not found",
                            }
                        )
                ...
                if not created:
                    return JsonResponse(
                        {"success": False, "error": f"{model_name} already exists"}
                    )
            ...
            return JsonResponse(
                {
                    ...
                    "name": obj.name,
                    "org_type": (
                        obj.org_type.name if model_name == "organization" else None
                    ),
                    "parent": (
                        obj.parent.name
...
def admin_sidebar_permissions(request):
    ...
    from .models import (OrganizationRole, OrganizationType, RoleAssignment,
                         SidebarPermission)
    ...
    SidebarPermission.objects.filter(user__isnull=False).exclude(
        role__in=["", None]
    ).delete()
    ...
    if org_type_id:
        ra_qs = RoleAssignment.objects.filter(
            role__organization__org_type_id=org_type_id
        )
        if org_role_id:
            ra_qs = ra_qs.filter(role_id=org_role_id)
        users_qs = users_qs.filter(
            id__in=ra_qs.values_list("user_id", flat=True)
        ).distinct()
    users = users_qs
    ...
    def build_assigned_tree(assigned_ids, items):
        ...
        for item in items:
            if "children" in item:
                ...
                if children:
                    result.append(
                        {"id": item["id"], "label": item["label"], "children": children}
                    )
            else:
    ...
    selected_role_id = (
        request.GET.get("role") or request.GET.get("org_role") or ""
    ).strip()
    ...
    if request.method == "POST":
        ...
        try:
            assigned_items = (
                json.loads(assigned_order_raw) if assigned_order_raw else []
            )
        except Exception:
            assigned_items = []
        ...
        if invalid_ids:
            messages.error(
                request, f"Unknown sidebar item(s): {', '.join(invalid_ids)}"
            )
        ...
        if target_users:
            for uid in target_users:
                if (
                    User.objects.filter(id=uid, is_superuser=True).exists()
                    and not assigned_items
                ):
                    messages.warning(
                        request,
    ...
    roles_qs = OrganizationRole.objects.select_related(
    ...
    context = {
        "organization_types": list(
            OrganizationType.objects.filter(is_active=True).values("id", "name")
        ),
        "org_roles": org_roles,
...
def enhanced_permissions_management(request):
    ...
    from .models import DashboardAssignment, OrganizationRole, OrganizationType

    # Get all data for the interface
    organization_types = OrganizationType.objects.all()
    roles = OrganizationRole.objects.select_related(
        "organization", "organization__org_type"
    ).all()
...
def api_save_dashboard_assignments(request):
    ...
    try:
        ...
        if not user_id and not role:
            return JsonResponse(
                {"success": False, "error": "Must specify either user or role"}
            )
        ...
        for dashboard in assignments:
            if user_id:
                DashboardAssignment.objects.create(
                    user_id=user_id, role="", dashboard=dashboard
                )
            else:
                DashboardAssignment.objects.create(
                    user=None, role=role, dashboard=dashboard
                )
...
def api_get_dashboard_assignments(request):
    ...
    if user_id:
        assignments = list(
            DashboardAssignment.objects.filter(
...
def api_my_sidebar(request):
    ...
    try:
        roles = RoleAssignment.objects.filter(user=request.user).select_related(
            "role", "organization", "organization__org_type"
        )
        roles_dbg = [
            {
...
def master_data_dashboard(request):
    ...
    return render(
        request,
        "core/master_data_dashboard.html",
        {
            "stats": stats,
            "recent_activities": recent_activities,
        },
...
def admin_approval_flow(request):
    ...
    org_types = OrganizationType.objects.filter(is_active=True).order_by("name")
    orgs_by_type = {
        ot.name: Organization.objects.filter(org_type=ot, is_active=True).order_by(
            "name"
        )
        for ot in org_types
    }
...
def admin_sdg_management(request):
    ...
    from .models import SDG_GOALS, SDGGoal
...
def admin_pso_po_management(request):
    ...
    for org_type in org_types:
        qs = Organization.objects.filter(org_type=org_type, is_active=True).order_by(
            "name"
        )
        orgs_by_type[org_type.name] = qs
        orgs_by_type_json[org_type.name.lower()] = [
            {"id": o.id, "name": o.name} for o in qs
        ]
    ...
    for program in programs:
        ...
        programs_data[program.id] = {
            ...
            "pos": [
                {"id": po.id, "description": po.description} for po in program.pos.all()
            ],
            "psos": [
                {"id": pso.id, "description": pso.description}
...
def manage_program_outcomes(request, program_id=None):
    ...
    if request.method == "POST":
        try:
            ...
            if outcome_type == "PO":
                outcome = ProgramOutcome.objects.create(
                    program=program, description=data["description"]
                )
        ...
        try:
            ...
            ProgramOutcome.DoesNotExist,
            ProgramSpecificOutcome.DoesNotExist,
            KeyError,
...
def create_program_for_organization(request):
    ...
    from .models import Organization, POPSOAssignment, Program
    ...
    try:
        ...
        if not (
            ...
            or POPSOAssignment.objects.filter(
                organization=organization, assigned_user=request.user, is_active=True
            ).exists()
        ):
            return JsonResponse(
...
def get_approval_flow(request, org_id):
    steps = ApprovalFlowTemplate.objects.filter(organization_id=org_id).order_by(
        "step_order"
    )
    ...
    return JsonResponse(
        {
            "success": True,
            "steps": data,
            "require_faculty_incharge_first": (
                config.require_faculty_incharge_first if config else False
            ),
        }
    )
...
def save_approval_flow(request, org_id):
    ...
    existing_steps = {
        s.step_order: s
        for s in ApprovalFlowTemplate.all_objects.filter(organization=org)
    }
    provided_orders = set()
...
def api_approval_flow_steps(request, org_id):
    steps = list(
        ApprovalFlowTemplate.objects.filter(organization_id=org_id)
        .order_by("step_order")
        .values("id", "step_order", "role_required", "user_id", "optional")
    )
...
def organization_users(request, org_id):
    ...
    if not assignments.exists() and org_type_id:
        assignments = RoleAssignment.objects.filter(
            organization__org_type_id=org_type_id
        )
    if role:
    ...
    assignments = assignments.select_related("user").order_by(
...
def api_org_type_organizations(request, org_type_id):
    ...
    orgs = Organization.objects.filter(
        org_type_id=org_type_id, is_active=True
    ).order_by("name")
...
def api_org_type_roles(request, org_type_id):
        ...
        return JsonResponse(
            {
                "success": False,
                "roles": [],
                "error": "No organizations found for this type",
            }
        )

    # Get the organization with the most roles (likely Commerce in your case)
    org_with_most_roles = (
        organizations.annotate(role_count=models.Count("organizationrole"))
        .order_by("-role_count")
        .first()
    )
    ...
    template_roles = (
        OrganizationRole.objects.filter(organization=org_with_most_roles)
    ...
    with transaction.atomic():
        for org in organizations:
            existing_role_names = set(
                OrganizationRole.objects.filter(organization=org).values_list(
                    "name", flat=True
                )
            )
    ...
    roles = (
        OrganizationRole.objects.filter(organization__org_type_id=org_type_id)
...
def api_organization_roles(request, org_id):
    ...
    roles = (
        OrganizationRole.objects.filter(organization_id=org_id, is_active=True)
        .values("id", "name")
        .order_by("name")
    )
    data = list(roles)
...
def api_filter_organizations(request):
    ...
    orgs = orgs.select_related("org_type").order_by("name")[
        :50
...
def api_filter_roles(request):
    ...
    roles = roles.select_related("organization").order_by("name")[
        :50
    ]  # Limit for performance

    data = [
        {"id": r.id, "name": r.name, "organization": r.organization.name} for r in roles
    ]
    return JsonResponse({"success": True, "roles": data})
...
def get_pso_po_data(request, org_type, org_id):
    ...
    try:
        ...
        if programs.exists():
            ...
            pos = list(
                ProgramOutcome.objects.filter(program=program).values(
                    "id", "description"
                )
            )
            psos = list(
        ...
        return JsonResponse(
            {"success": True, "pos": pos, "psos": psos, "organization": org.name}
        )
    except Organization.DoesNotExist:
...
def add_outcome(request, outcome_type):
    ...
    try:
        ...
        program, created = Program.objects.get_or_create(
            organization=org, defaults={"name": f"{org.name} Program"}
        )
...
def api_auth_me(request):
    ...
    initials = (
        "".join([x[0] for x in user.get_full_name().split()])
...
def api_dashboard_places(request):
    ...
    venues = (
        EventProposal.objects.filter(venue__isnull=False)
        .exclude(venue="")
        .values_list("venue", flat=True)
        .distinct()
    )

    places = [{"id": f"venue_{i}", "name": venue} for i, venue in enumerate(venues, 1)]
...
def api_dashboard_people(request):
    ...
    user_orgs = user.role_assignments.filter(organization__isnull=False).values_list(
        "organization_id", flat=True
    )
    ...
    if org_id:
        ...
        try:
            ...
            if org_id in user_orgs:
                users = (
                    User.objects.filter(role_assignments__organization_id=org_id)
                    .exclude(id=user.id)
                    .distinct()
                )
            else:
        ...
        users = (
            User.objects.filter(role_assignments__organization_id__in=user_orgs)
            .exclude(id=user.id)
            .distinct()
        )
    ...
    people = [
        {
            ...
            "email": u.email,
            "role": (
                getattr(u.profile, "role", "user") if hasattr(u, "profile") else "user"
            ),
        }
...
def api_user_proposals(request):
    ...
    proposals = EventProposal.objects.filter(submitted_by=request.user).order_by(
        "-created_at"
    )[:50]
        ...
        proposal_data.append(
            {
                ...
                "view_url": reverse(
                    "emt:proposal_status_detail", kwargs={"proposal_id": p.id}
                ),
            }
        )
...
def api_student_contributions(request):
    ...
    date_window = (
        models.Q(
            event_datetime__date__gte=start_date, event_datetime__date__lte=end_date
        )
    ...
    contributions_map = {}
    daily = (
        qs.annotate(date=date_expr).values("date").annotate(count=models.Count("id"))
    )
    for row in daily:
        date_obj = row["date"]
        date_str = (
...
def api_user_events_data(request):
    ...
    events = EventProposal.objects.filter(submitted_by=user).order_by("-created_at")[
        :10
    ]
        ...
        events_data.append(
            {
                ...
                "organization": (
                    event.organization.name if event.organization else "N/A"
                ),
            }
        )
...
def api_student_performance_data(request):
    ...
    participated_events = (
        EventProposal.objects.filter(
            organization_id__in=filter_orgs, status__in=["approved", "finalized"]
        )
        .exclude(submitted_by=user)
        .count()
    )  # Simplified - in real app you'd have participants table
...
def api_calendar_events(request):
    ...
    def build_view_url(item_id):
        ...
        if request.user.is_superuser:
            try:
                return reverse(
                    "emt:proposal_status_detail", kwargs={"proposal_id": item_id}
                )
            except Exception:
                pass
    ...
    if category in ("all", "public"):
        ...
        for e in events:
            ...
            base_data = {
                ...
                "organization": (
                    e.organization.name if getattr(e, "organization", None) else None
                ),
                "submitted_by": (
                    e.submitted_by.get_full_name()
                    if getattr(e, "submitted_by", None)
                    else None
                ),
                "venue": e.venue or "",
                "type": "public",
                ...
                "event_start_date": (
                    e.event_start_date.isoformat() if e.event_start_date else None
                ),
                "event_end_date": (
                    e.event_end_date.isoformat() if e.event_end_date else None
                ),
            }
    ...
    if category == "faculty":
        ...
        try:
            has_faculty_role = any(
                (ra.role and ra.role.name.lower() == "faculty")
                for ra in user.role_assignments.select_related("role")
            )
        except Exception:
            has_faculty_role = False
        ...
        org_ids = list(
            user.role_assignments.filter(organization__isnull=False).values_list(
                "organization_id", flat=True
            )
        )
            ...
            items.append(
                {
                    ...
                    "view_url": "",
                    "gcal_url": gcal_quick_add_link(
                        m.title, m.scheduled_at, None, m.description
                    ),
                }
            )
...
def gcal_quick_add_link(
    title: str, start_dt, end_dt=None, description: str = ""
) -> str:
    try:
        import urllib.parse as ul
    except Exception:
...
def api_create_faculty_meeting(request):
    ...
    try:
        payload = (
            json.loads(request.body.decode("utf-8")) if request.body else request.POST
        )
    except Exception:
    ...
    if not org_id or not when:
        return JsonResponse(
            {
                "success": False,
                "error": "organization_id and scheduled_at are required",
            },
            status=400,
        )
    ...
    try:
        ...
        return JsonResponse(
            {"success": False, "error": "Invalid organization_id"}, status=400
        )
    user_org_ids = set(
        user.role_assignments.filter(organization__isnull=False).values_list(
            "organization_id", flat=True
    ...
    try:
        ...
        if dt is None:
            return JsonResponse(
                {"success": False, "error": "Invalid datetime"}, status=400
            )

        participants_text = ", ".join(participants) if participants else "All faculty"
        description_text = (
...
def api_global_search(request):
    ...
    try:
        ...
        try:
            ...
            ).filter(profile__isnull=False)[:5]
            results["students"] = [
                {
                    "id": user.id,
            ...
            results["proposals"] = [
                {
                    ...
                    "faculty": (
                        proposal.submitted_by.get_full_name()
                        if proposal.submitted_by
                        else "N/A"
                    ),
                    "organization": (
                        proposal.organization.name if proposal.organization else "N/A"
                    ),
                    "status": getattr(proposal, "status", "Unknown"),
                    "date": (
                        proposal.event_datetime.strftime("%Y-%m-%d")
            ...
            results["reports"] = [
                {
                    ...
                    "organization": (
                        report.organization.name if report.organization else "N/A"
                    ),
            ...
            results["users"] = [
                {
                    ...
                    "email": user.email,
                    "role": (
                        getattr(user.profile, "role", "User")
...
def _emt_event_to_dict(ev: EMTEventProposal):
    ...
    return {
        "title": getattr(ev, "event_title", "")
        or getattr(ev, "title", "")
        or f"Event #{ev.id}",
        "status": getattr(ev, "status", None)
        or getattr(ev, "get_status_display", lambda: "")(),
...
def _core_event_to_dict(ev):
    ...
    org = getattr(ev, "organization", None)
    return {
        "title": getattr(ev, "event_title", None)
        or getattr(ev, "title", None)
        or f"Event #{ev.id}",
        "status": getattr(ev, "status", None),
        "submitted_by": (
            ev.submitted_by.get_full_name()
            if getattr(ev, "submitted_by", None)
            else None
        ),
        "organization": org.name if org else None,
...
def _report_to_dict(r: Report):
    ...
    return {
        ...
        "submitted_by": (
            r.submitted_by.get_full_name() if getattr(r, "submitted_by", None) else None
        ),
        "organization": org.name if org else None,
        ...
        if t == "organization_type":
            ...
            if start and end:
                ...
                if hasattr(EMTEventProposal, "event_start_date"):
                    qs = qs.filter(
                        event_start_date__gte=start, event_start_date__lte=end
                    )
                else:
    ...
    for f in filters:
        ...
        if t == "organization_type":
            try:
                ot = int(v)
                qs = qs.filter(
                    role_assignments__organization__org_type_id=ot
                ).distinct()
            except Exception:
                pass
            ...
            try:
                ...
                qs = qs.filter(
                    role_assignments__organization__name__icontains=str(v)
                ).distinct()
...
def build_organization_queryset(q=None, filters=None):
    ...
    if q:
        s = q.strip()
        qs = qs.filter(Q(name__icontains=s) | Q(org_type__name__icontains=s))
    if not filters:
        return qs
    for f in filters:
        t = f.get("type")
...
def api_org_types(request):
    ...
    data = [
        {"id": ot.id, "name": ot.name, "is_active": getattr(ot, "is_active", True)}
        for ot in qs
    ]
...
def api_orgs_by_type(request):
    ...
    try:
        ...
        data = [
            {"id": o.id, "name": o.name, "is_active": getattr(o, "is_active", True)}
            for o in qs
        ]
        return JsonResponse(data, safe=False)
    except Exception:
...
def api_filter_meta(request, category):
    ...
    if category == "events":
        ...
        try:
            statuses = [
                {
                    "value": s.value if hasattr(s, "value") else s[0],
                    "label": s.label if hasattr(s, "label") else s[1],
                }
                for s in EMTEventProposal.Status
            ]
        except Exception:
            ...
            try:
                statuses = [
                    {"value": s[0], "label": s[1]}
                    for s in EMTEventProposal.STATUS_CHOICES
                ]
            except Exception:
                statuses = []
        return JsonResponse(
            {
                ...
                "filters": {
                    "organization_types": [
                        {"id": ot.id, "name": ot.name}
                        for ot in OrganizationType.objects.filter(is_active=True)
                    ],
                    "statuses": statuses,
        ...
        report_types = (
            [{"value": r[0], "label": r[1]} for r in Report.REPORT_TYPE_CHOICES]
            if hasattr(Report, "REPORT_TYPE_CHOICES")
            else []
        )
        status_choices = (
            [{"value": s[0], "label": s[1]} for s in Report.STATUS_CHOICES]
        ...
        return JsonResponse(
            {
                ...
                "filters": {
                    "organization_types": [
                        {"id": ot.id, "name": ot.name}
                        for ot in OrganizationType.objects.all()
                    ],
                    "active_flags": ["active", "inactive"],
                },
            }
...
def api_search(request):
    ...
    q = (
        payload.get("q", payload.get("q", "")).strip()
    ...
    page_size = int(
        payload.get("page_size", request.GET.get("page_size", 1000000) or 1000000)
    )
    ...
    if category == "events":
        qs = build_event_queryset(q=q or None, filters=filters or None).order_by(
            "-created_at"
        )
        total = qs.count()
        ...
        include_core = any(
            f.get("type") == "include_core"
            and f.get("value") in (True, "true", "True", 1, "1")
            for f in filters
        )
        ...
        qs = build_user_queryset(q=q or None, filters=filters or None).order_by(
            "-date_joined"
        )
        total = qs.count()
        start = (page - 1) * page_size
        ...
        # Core Reports
        qs = build_report_queryset(q=q or None, filters=filters or None).order_by(
            "-created_at"
        )
        total = qs.count()
            ...
            results.append(
                {
                    ...
                    "submitted_by": (
                        r.submitted_by.get_full_name() if r.submitted_by else None
                    ),
                    "status": r.status,
        ...
        # EMT Event Reports
        evr_qs = EMTEventReport.objects.select_related(
            "proposal", "proposal__organization", "proposal__submitted_by"
        )
        ...
        for evr in evr_qs:
            results.append(
                {
                    "title": (
                        evr.proposal.event_title if evr.proposal else "Untitled Event"
                    ),
                    "report_type": "Event Report",
                    "organization": (
                        evr.proposal.organization.name
                        if evr.proposal and evr.proposal.organization
                        else None
                    ),
                    "submitted_by": (
                        evr.proposal.submitted_by.get_full_name()
                        if evr.proposal and evr.proposal.submitted_by
                        else None
                    ),
                    "status": "Generated",
                    "file": None,
        ...
        qs = build_organization_queryset(q=q or None, filters=filters or None).order_by(
            "org_type__name", "name"
        )
        total = qs.count()
        start = (page - 1) * page_size
        ...
        return JsonResponse(
            {"error": "Invalid category. Choose events/users/reports/organizations"},
            status=400,
        )
...
def api_export_csv(request):
    ...
    payload.setdefault(
        "page_size", 1000000
    )  # export everything by default (cap by server memory)
    ...
    if not items:
        ...
        response["Content-Disposition"] = (
            f'attachment; filename="export_{category}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.csv"'
        )
...
def api_export_excel(request):
    ...
    if xlsxwriter is None:
        return JsonResponse(
            {"error": "XLSX export not available (xlsxwriter missing)"}, status=501
        )
    try:
        payload = json.loads(request.body.decode("utf-8") or "{}")
    except Exception:
    ...
    if category == "events":
        objs = [
            _emt_event_to_dict(e)
            for e in build_event_queryset(
                q=q or None, filters=filters or None
            ).order_by("-created_at")
        ]
    elif category == "users":
        objs = [
            _user_to_dict(u)
            for u in build_user_queryset(q=q or None, filters=filters or None).order_by(
                "-date_joined"
            )
        ]
    elif category == "reports":
        objs = [
            _report_to_dict(r)
            for r in build_report_queryset(
                q=q or None, filters=filters or None
            ).order_by("-created_at")
        ]
    elif category == "organizations":
        objs = [
            _org_to_dict(o)
            for o in build_organization_queryset(
                q=q or None, filters=filters or None
            ).order_by("org_type__name", "name")
        ]
    else:
    ...
    response["Content-Disposition"] = (
        f'attachment; filename="export_{category}_{datetime.now().strftime("%Y%m%d_%H%M%S")}.xlsx"'
    )
    return response
...
def api_quick_summary(request):
    ...
    if ot:
        try:
            otid = int(ot)
            ev_count = EMTEventProposal.objects.filter(
                organization__org_type_id=otid
            ).count()
            ...
            user_count = (
                User.objects.filter(role_assignments__organization__org_type_id=otid)
                .distinct()
                .count()
            )
        except Exception:
    ...
    if request.method == "POST":
        try:
            ...
            users = (
                ...
                .exclude(id=request.user.id)  # Don't include current user
...
def impersonate_user(request):
    ...
    try:
        ...
        messages.success(
            request,
            f"Now viewing as: {target_user.get_full_name() or target_user.username}",
        )
...
def student_event_details(request, proposal_id):
    ...
    user_can_view = (
        ...
        or request.user in proposal.faculty_incharges.all()
        or request.user.is_superuser
        or proposal.status == "finalized"  # Anyone can view finalized events
    )
...
def api_organization_programs(request, org_id):
    ...
    try:
        programs = Program.objects.filter(organization_id=org_id).values("id", "name")
...
def api_program_outcomes(request, program_id):
    ...
    try:
        ...
        if outcome_type == "PO":
            base_qs = (
                ProgramOutcome.all_objects if show_archived else ProgramOutcome.objects
            )
            ...
            base_qs = (
                ProgramSpecificOutcome.all_objects
                if show_archived
                else ProgramSpecificOutcome.objects
            )
            outcomes = base_qs.filter(program_id=program_id)
            if show_archived:
                outcomes = outcomes.filter(status="archived")
            ...
            program = Program.objects.get(id=program_id)
            if show_archived:
                pos_qs = ProgramOutcome.all_objects.filter(
                    program=program, status="archived"
                )
                pso_qs = ProgramSpecificOutcome.all_objects.filter(
                    program=program, status="archived"
                )
            else:
            ...
            pos = [
                {
                    ...
                    "status": getattr(po, "status", "active"),
                }
                for po in pos_qs
            ]
            psos = [
                {
                    ...
                    "status": getattr(pso, "status", "active"),
                }
                for pso in pso_qs
            ]
            return JsonResponse(
                {
                    "program": {
                        ...
                        "name": program.name,
                        "organization": (
                            program.organization.name if program.organization else None
                        ),
                    },
                    "pos": pos,
                    "psos": psos,
        ...
        logger.error(
            f"Error fetching program outcomes for program {program_id}: {str(e)}"
        )
...
def api_available_faculty_users(request, org_id):
    ...
    try:
        ...
        logger.info(
            f"Loading faculty for org: {organization.name} (Type: {organization.org_type.name})"
        )
        ...
        logger.info(
            "Found %s FACULTY users for org %s ('%s') with search '%s'",
            len(users_data),
            org_id,
            organization.name,
            search_query,
...
def api_debug_org_users(request, org_id):
    ...
    try:
        ...
        all_users = User.objects.filter(
            role_assignments__organization=organization, is_active=True
        ).distinct()
        ...
        for user in all_users:
            user_roles = list(
                user.role_assignments.filter(organization=organization).values_list(
                    "role__name", flat=True
                )
            )
...
def api_popso_assignments(request, org_id=None):
    ...
    if request.method == "GET":
        try:
            if org_id:
                    ...
                    return JsonResponse(
                        {
                            "assigned_user": {
                                ...
                                "full_name": assignment.assigned_user.get_full_name()
                                or assignment.assigned_user.username,
                ...
                assignments = POPSOAssignment.objects.filter(
                    is_active=True
                ).select_related("organization", "assigned_user", "assigned_by")
        ...
        try:
            ...
            logger.warning("Invalid data when creating assignment", exc_info=exc)
            return JsonResponse({"success": False, "error": "Invalid data"}, status=400)
        except Exception as e:
            logger.error(f"Error creating assignment: {str(e)}")
            return JsonResponse(
                {"success": False, "error": "Failed to create assignment"}, status=500
            )
        ...
        try:
            if not org_id:
                return JsonResponse(
                    {"success": False, "error": "Organization ID required"}, status=400
                )
            ...
            return JsonResponse(
                {"success": False, "error": "Failed to remove assignment"}, status=500
            )
...
def api_log_popso_change(request):
    ...
    try:
        ...
        logger.warning("Invalid data when logging PO/PSO change", exc_info=exc)
        return JsonResponse({"success": False, "error": "Invalid data"}, status=400)
    except Exception as e:
        logger.error(f"Error logging PO/PSO change: {str(e)}")
        return JsonResponse(
            {"success": False, "error": "Failed to log change"}, status=500
        )
...
def cdl_head_dashboard(request):
    ...
    if (
        request.user.is_superuser
        or request.user.groups.filter(name="CDL_HEAD").exists()
    ):
        is_allowed = True
    else:
        try:
            ...
            ras = RoleAssignment.objects.filter(user=request.user).select_related(
                "role", "organization", "organization__org_type"
            )
            for ra in ras:
    ...
    from emt.models import CDLSupport, EventProposal
    ...
    total_active_requests = EventProposal.objects.filter(
        status__in=active_statuses
    ).count()

    # Assets Delivery Pending - Events with CDL support where assets are not yet delivered
    assets_pending = (
        CDLSupport.objects.filter(
            proposal__status__in=["approved", "finalized"], needs_support=True
        )
        .filter(Q(poster_required=True) | Q(certificates_required=True))
        .count()
    )

    # Unassigned Tasks - Events that need CDL support but don't have assignees yet
    unassigned_tasks = CDLSupport.objects.filter(
        needs_support=True, proposal__status__in=active_statuses
    ...
    calendar_events = []
    proposals_with_cdl = (
        EventProposal.objects.filter(cdl_support__needs_support=True)
        .select_related("cdl_support")
            ...
            calendar_events.append(
                {
                    ...
                    "organization": (
                        proposal.organization.name if proposal.organization else ""
                    ),
                    "type": "cdl_support",
                    "poster_required": (
                        proposal.cdl_support.poster_required
                        if hasattr(proposal, "cdl_support")
                        else False
                    ),
                    "certificates_required": (
                        proposal.cdl_support.certificates_required
                        if hasattr(proposal, "cdl_support")
                        else False
                    ),
                }
...
def cdl_member_dashboard(request):
    if not (
        request.user.is_superuser
        or request.user.groups.filter(name="CDL_MEMBER").exists()
    ):
        return HttpResponseForbidden()
    ctx = {
        "member_inbox": [],
    ...
    if (
        ...
        try:
            ...
            for ra in ras:
                ...
                if org_type == "cdl" and role_name in {
                    "cdl employee",
                    "cdl member",
                    "cdl team",
                }:
...
def cdl_communication_page(request):
    ...
    if not is_allowed:
        try:
            ...
            for ra in RoleAssignment.objects.filter(user=request.user).select_related(
                "role", "organization", "organization__org_type"
            ):
                role_name = (ra.role.name if ra.role else "").lower()
                org_type = (
                ...
                if org_type == "cdl" and any(
                    k in role_name for k in ["head", "employee", "member", "team"]
                ):
                    is_allowed = True
                    break
        except Exception:
    ...
    # Additional conditional: allow event proposer or faculty-incharge
    # for a specific event if the event has any CDL related support
    # requirements so they can coordinate.
    if not is_allowed:
        event_id = request.GET.get("eventId") or request.GET.get("proposal_id")
        if event_id and event_id.isdigit():
            from emt.models import \
                EventProposal  # local import to avoid circulars when app not loaded

            try:
                ep = (
                    EventProposal.objects.select_related("organization")
                    .prefetch_related("faculty_incharges")
                    .get(id=int(event_id))
                )
                # Determine if CDL support flags exist via related cdl_support
                # (if present) or fallbacks used earlier in APIs
                cdl_flags = False
                try:
                    cs = getattr(ep, "cdl_support", None)
                    if cs and (
                        getattr(cs, "poster_required", False)
                        or getattr(cs, "certificates_required", False)
                    ):
                        cdl_flags = True
                except Exception:
                    pass
                # If we cannot find cdl_support, still allow if the proposal is finalized
                # (assuming potential design assets).
                if not cdl_flags and ep.status in ("finalized", "approved"):
                    cdl_flags = True
                if cdl_flags:
                    # Is the user proposer or faculty incharge?
                    if (
                        ep.submitted_by_id == request.user.id
                        or ep.faculty_incharges.filter(id=request.user.id).exists()
                    ):
                        is_allowed = True
            except EventProposal.DoesNotExist:
                pass
...
def api_cdl_communication(request):
    ...
    if request.method == "POST":
        ...
        if not comment and "attachment" not in request.FILES:
            return JsonResponse(
                {"success": False, "error": "Empty message"}, status=400
            )
...
def api_cdl_support_detail(request, proposal_id: int):
    try:
        p = (
            EMTEventProposal.objects.select_related(
                "organization", "submitted_by", "cdl_support"
            )
    ...
    data = {
        ...
        "venue": p.venue,
        "date": (
            p.event_datetime.date().isoformat()
            if p.event_datetime
            else (p.event_start_date.isoformat() if p.event_start_date else None)
        ),
        "submitted_by": p.submitted_by.get_full_name() or p.submitted_by.username,
        "submitter_email": p.submitted_by.email,
        "faculty_incharges": [
            u.get_full_name() or u.username for u in p.faculty_incharges.all()
        ],
        "speakers": speakers,
        # support details
        ...
        "cdl_completed_at": (
            getattr(s, "completed_at", None).isoformat()
        ...
        # assignment details
        "assigned_to_id": (
            asg.assignee_id if asg else getattr(p, "report_assigned_to_id", None)
        ),
        "assigned_to_name": (
            (asg.assignee.get_full_name() or asg.assignee.username)
            if asg
            else (p.report_assigned_to.get_full_name() if p.report_assigned_to else None)
        ),
...
def api_cdl_support_assign(request, proposal_id: int):
    ...
    if not member_id:
        return JsonResponse(
            {"success": False, "error": "member_id is required"}, status=400
        )
...
def api_cdl_support_resources(request, proposal_id: int):
    ...
    role_asg = RoleAssignment.objects.filter(
        organization__org_type__in=org_types
    ).select_related("user", "role")
    members = []
    seen = set()
    for ra in role_asg:
        ...
        if ("employee" in rn) or ("member" in rn):
            ...
            seen.add(uid)
            members.append(
                {"id": uid, "name": ra.user.get_full_name() or ra.user.username}
            )
    ...
    for key in list(existing.keys()):
        if not any(r["key"] == key for r in resources):
            resources.append(
                {"key": key, "label": key.replace("_", " ").title(), "custom": True}
            )
...
def api_cdl_save_task_assignments(request, proposal_id: int):
    ...
    if isinstance(to_unassign, list) and to_unassign:
        qs = _Task.objects.filter(
            proposal=p, resource_key__in=[(str(k) or "").strip() for k in to_unassign]
        )
...
@require_http_methods(
    ["POST", "PUT", "DELETE"]
)  # /api/cdl/support/<proposal_id>/tasks/
def api_cdl_tasks_crud(request, proposal_id: int):
    ...
    if request.method == "POST":
        ...
        if not res_key:
            return JsonResponse(
                {"success": False, "error": "resource is required"}, status=400
            )
        ...
        return JsonResponse(
            {
                ...
                "task": {
                    "id": obj.id,
                    "resource": obj.resource_key,
                    "label": obj.label or obj.resource_key,
                    "status": obj.status,
                    "assignee_id": obj.assignee_id,
                    "assignee_name": (
                        (obj.assignee.get_full_name() or obj.assignee.username)
                        if obj.assignee_id
                        else None
                    ),
                },
    ...
    if request.method == "PUT":
        ...
        if not is_cdl_head:
            ...
            if obj.assignee_id != request.user.id:
                return JsonResponse(
                    {"success": False, "error": "Not allowed"}, status=403
                )
            ...
            if "assignee_id" in payload:
                ...
                if aid:
                    try:
                        obj.assignee = User.objects.get(id=int(aid))
                    except Exception:
                        obj.assignee = None
                else:
                    obj.assignee = None
    ...
    if request.method == "DELETE":
        ...
        if tid:
            qs = qs.filter(id=int(tid))
        elif res_key:
            qs = qs.filter(resource_key=str(res_key))
        else:
            return JsonResponse(
                {"success": False, "error": "id or resource required"}, status=400
            )
        deleted = list(qs.values_list("id", flat=True))
        qs.delete()
...
def api_cdl_member_work(request):
    ...
    qs = (
        ...
        .filter(report_assigned_to=me)
        .select_related("organization", "cdl_support")
        .exclude(cdl_support__completed=True)
    )[:100]
        ...
        items.append(
            {
                ...
                "type": (
                    "poster"
                    if getattr(getattr(p, "cdl_support", None), "poster_required", False)
                    else (
                        "certificate"
                        if getattr(getattr(p, "cdl_support", None), "certificates_required", False)
                        else "coverage"
                    )
                ),
                "priority": "Medium",
                "due_date": (
                    p.event_start_date.isoformat() if p.event_start_date else None
                ) or (
                    p.event_datetime.date().isoformat() if p.event_datetime else None
                ),
                "status": "pending",
                "rev": 0,
                "created_at": p.created_at.isoformat(),
                "assigned_at": (
                    p.report_assigned_at.isoformat() if p.report_assigned_at else None
                ),
            }
        )
    return JsonResponse({"success": True, "items": items})
...
def api_cdl_members(request):
    ...
    qs = User.objects.filter(groups__name="CDL_MEMBER", is_active=True).order_by(
    ...
    for ra in role_asg:
        ...
        if key:
            ...
            users[key].append(
                {
                    "id": uid,
                    "name": ra.user.get_full_name() or ra.user.username,
                    "role": ra.role.name,
                }
            )
...
def api_cdl_member_data(request):
    ...
    asg_ids = list(
        _CDLAssignment.objects.filter(assignee=user).values_list("proposal_id", flat=True)
    )
    ...
    task_qs = (
        _Task.objects
        .filter(assignee=user, proposal__cdl_support__completed=False)
        ...
        work.append(
            {
                ...
                "assigned_role": label,
                "assigned_by": (
                    (t.assigned_by.get_full_name() or t.assigned_by.username)
                    if t.assigned_by
                    else None
                ),
            }
        ...
        work.append(
            {
                ...
                "assigned_by": (
                    (_asg.assigned_by.get_full_name() or _asg.assigned_by.username)
                    if (_asg and _asg.assigned_by)
                    else None
                ),
            }
        )
    ...
    events_all_qs = (
        ...
        .filter(
            ...
            | models.Q(event_start_date__isnull=False)
        )
        .select_related("organization")
...
def api_cdl_member_accept(request):
    ...
    from emt.models import CDLAssignment as _CDLAssignment
    from emt.models import EMTEventProposal as _Proposal

    try:
        prop = _Proposal.objects.get(id=pid)
    except _Proposal.DoesNotExist:
        return JsonResponse(
            {"success": False, "error": "Proposal not found"}, status=404
        )
    ...
    if not asg:
        ...
        if getattr(prop, "report_assigned_to_id", None) == me.id:
            ...
            if has_task:
                ...
                return JsonResponse(
                    {"success": False, "error": "Not assigned to you"}, status=403
                )
...
def api_update_avatar(request):
    ...
    try:
        ...
        if avatar.content_type not in allowed_types:
            return JsonResponse(
                {"success": False, "message": "Only JPEG and PNG files are allowed"},
                status=400,
            )
...
def api_get_user_events(request):
    ...
    try:
        ...
        if hasattr(user, "student"):
            ...
            events = (
                EventProposal.objects.filter(Q(students=student) | Q(created_by=user))
                .distinct()
                .order_by("-created_at")
            )
            ...
            events = EventProposal.objects.filter(created_by=user).order_by(
                "-created_at"
            )
    ...
    try:
        ...
        if request.method == "GET":
            ...
            if not achievement_id:
                return JsonResponse(
                    {"success": False, "message": "Achievement ID required"}, status=400
                )
            ...
            try:
                ...
                return JsonResponse(
                    {"success": False, "message": "Invalid achievement ID"}, status=400
                )
    ...
    try:
        ...
        if "enrollment_year" in data:
            student.enrollment_year = (
                int(data["enrollment_year"]) if data["enrollment_year"] else None
            )

        student.save()

        return JsonResponse(
            {"success": True, "message": "Academic information updated successfully"}
        )
