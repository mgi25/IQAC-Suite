...
def preview_event_report(request, proposal_id):
    ...
    def _field(label, value, *, multiline=False, muted=False):
        return {
            "label": label,
            "value": _text(value),
            "multiline": multiline,
    ...
    event_data = initial_data.get("event", {})
    participants_data = initial_data.get("participants", {})
    organising_committee = participants_data.get("organising_committee", {})
    narrative_data = initial_data.get("narrative", {})
    analysis_data = initial_data.get("analysis", {})
    mapping_data = initial_data.get("mapping", {})
    metrics_data = initial_data.get("metrics", {})
    iqac_data = initial_data.get("iqac", {})

    overview_highlights = []
    highlight_candidates = [
        ("Department", event_data.get("department")),
        ("Event Dates", event_data.get("date")),
        ("Venue", event_data.get("venue")),
        ("Academic Year", event_data.get("academic_year")),
        ("Event Type", event_data.get("event_type_focus")),
        ("Activities", event_data.get("no_of_activities")),
    ]
    for label, value in highlight_candidates:
        text_value = _text(value)
        if text_value:
            overview_highlights.append({"label": label, "value": text_value})

    social_relevance_items = _clean_list(narrative_data.get("social_relevance"))
    outcomes_items = _clean_list(narrative_data.get("outcomes"))
    committee_items = _clean_list(
        organising_committee.get("event_coordinators", []) if organising_committee else []
    )

    naac_tags_cleaned = _clean_list(metrics_data.get("naac_tags"))

    attachments_completed = [
        checklist_labels.get(key, key)
        for key, value in attachments_checklist.items()
        if value
    ]
    attachments_missing = [
        checklist_labels.get(key, key)
    ...
    photo_summaries = []
    for photo in annexure_photos:
        caption = _text(photo.get("caption"))
        if caption:
            photo_summaries.append(caption)
        else:
            src = _text(photo.get("src"))
            if src:
                photo_summaries.append(src)
    ...
    preview_sections = [
        {
            ...
            "fields": [
                _field("Department", event_data.get("department")),
                _field("Event Title", event_data.get("title")),
                _field("No. of Activities", event_data.get("no_of_activities")),
                _field("Event Dates", event_data.get("date")),
                _field("Venue", event_data.get("venue")),
                _field("Academic Year", event_data.get("academic_year")),
                _field("Event Type (Focus)", event_data.get("event_type_focus")),
                _field("Blog Link", event_data.get("blog_link")),
            ],
        },
        {
            "id": "participants",
            ...
            "fields": [
                _field("Target Audience", participants_data.get("target_audience")),
                _field(
                    "External Agencies / Speakers",
                    participants_data.get("external_agencies_speakers"),
                ),
                _field(
                    "External Contacts",
                    participants_data.get("external_contacts"),
                ),
                _field(
                    "No. of Student Volunteers",
                    organising_committee.get("student_volunteers_count") if organising_committee else "",
                ),
                _field(
                    "No. of Attendees / Participants",
                    participants_data.get("attendees_count"),
                ),
            ],
            "lists": [
                {
        ...
        },
        {
            ...
            "fields": [
                _field(
                    ...
                    narrative_data.get("summary_overall_event"),
                    multiline=True,
                ),
            ],
            "lists": [
                {
                    "label": "Social Relevance",
                    "items": social_relevance_items,
                },
                {
                    "label": "Outcomes of the Event",
        ...
        },
        {
            ...
            "fields": [
                _field("Impact on Attendees", analysis_data.get("impact_attendees"), multiline=True),
                _field("Impact on Schools / Departments", analysis_data.get("impact_schools"), multiline=True),
                _field("Impact on Volunteers", analysis_data.get("impact_volunteers"), multiline=True),
            ],
        },
        {
            "id": "mapping",
            ...
            "fields": [
                _field("POs / PSOs", mapping_data.get("pos_psos"), multiline=True),
                _field(
                    "Graduate Attributes / Needs",
                    mapping_data.get("graduate_attributes_or_needs"),
                    multiline=True,
                ),
                _field(
                    "Contemporary Requirements",
                    mapping_data.get("contemporary_requirements"),
                    multiline=True,
                ),
                _field("Value Systems", mapping_data.get("value_systems"), multiline=True),
            ],
            "lists": [
                {
                    "label": "SDG Goal Numbers",
                    "items": _clean_list(mapping_data.get("sdg_goal_numbers")),
                }
            ],
            "chips": naac_tags_cleaned,
        },
        {
            "id": "iqac",
            "title": "IQAC Review",
            "fields": [
                _field("Review Date", iqac_data.get("iqac_review_date")),
                _field("Head / Coordinator", iqac_data.get("sign_head_coordinator")),
                _field(
                    "Faculty Coordinator / Organiser",
                    iqac_data.get("sign_faculty_coordinator"),
                ),
            ],
            "lists": [
                {
                    "label": "Suggestions for Improvement",
                    "items": _clean_list(iqac_data.get("iqac_suggestions")),
                }
            ],
        },
        {
            ...
            "fields": [
                _field("Uploaded Photo Evidence", str(len(photo_summaries)) if photo_summaries else "0"),
            ],
            "lists": [
                {"label": "Provided", "items": attachments_completed},
                {"label": "Pending", "items": attachments_missing},
                {"label": "Photo Highlights", "items": photo_summaries},
            ],
        },
    ]

    preview_nav = [{"id": section["id"], "title": section["title"]} for section in preview_sections]
